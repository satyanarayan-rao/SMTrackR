fp_info_for_complete_molecule  <- function (footprint_string = "F...FFFF...FFF.....FFFFF."){
    # use stringr library
...
fp_info_for_sub_molecule <- function (query_footprint = "FF...FFF.....FF",
                                      query_start = 7,
                                      query_end = 21,
                                      complete_footprint = "F...FFFF...FFF.....FFFFF."){
    fp_start_loci_on_query = locate_fp_start(footprint_string = query_footprint)
    ...
    for (i in fp_start_loci_on_query){
        ...
        fp_len_query = c (fp_len_query, fp_len_at_each_pos[idx_on_long_molecule])
    }
...
extend_fp_from_center <- function (sub_moleculue = "FF...FFF.....FF",
                                   complete_molecule = "F...FFFF...FFF.....FFFFF.",
                                   abs_roi_start = 7,
                                   abs_roi_end = 21,
                                   abs_complete_molecule_start = 1,
                                   abs_complete_molecule_end = 25,
                                   lextend = 30,
                                   rextend = 30,
                                   strand = "+") {
    # center of the submolecule = (7 + 21)/2 = 14
    # lextend  = 20, rextend = 0;
    # would add 6M in the left and keep right as it is.
    #
    ...
    m_to_add_in_right = abs ( min (len_complete_molecule - (peak_center + rextend), 0 ) )
    #print (c(rel_roi_start, rel_roi_end, peak_center, len_complete_molecule,
    #         m_to_add_in_left, m_to_add_in_right))
    ...
    final_string = paste0(paste0(rep("M", m_to_add_in_left), collapse = ""),
                          ...
                          collapse = "")
    #print (str_length(final_string))
    return(final_string)
    # if ( (lextend + rextend + 1)%%2==0){
    #    print ("Here")
    #   return (final_string)
    # }else {
    #   print ("There")
    #   if (m_to_add_in_left > 0 || (m_to_add_in_left == 0 && extended_sub_molecule_left == 1)){
    #     final_string = paste0("M", final_string)
    #     return (final_string)
    #   }else {
    #     # add one base in left
    #     extended_sub_molecule = str_sub(complete_molecule,
    #                                     extended_sub_molecule_left - 1,
    #                                     extended_sub_molecule_right)
    #
...
add_footprints_on_edges <- function(methylation_string = "..Z..x..Z...",
    ...
    if (left_border ==0  && right_border == 0) {
        ...
        if (left_border > 0 && right_border > 0) {
            o_str <- paste0(
                ...
                substr(footprint_string, (left_border + 1), (right_border - 1) ),
                paste(rep(rep_v, (len_vec - right_border + 1) ), collapse = "")
            )
        }else{
            if (left_border > 0 && right_border == 0){
                o_str = paste0(
                    paste(rep(rep_v, left_border), collapse = ""),
                    substr(footprint_string, (left_border + 1) , len_vec), collapse ="")
            }else{
                if (left_border == 0 && right_border > 0){
    ...
    #return(list (o_str = o_str, left_border = left_border, right_border = right_border))
    return(o_str)
}
...
capital_percentage_and_stretch_of_unbound <- function (methylation_vector = ""){
    ...
    if (first_found == T) {
        if (last_capital == -1){
            ...
            naked_dna_stretches = c (naked_dna_stretches, left_extreme)
            naked_dna_stretches = c (naked_dna_stretches, last_capital - left_extreme) # check if -1 is required
            naked_dna_stretches = c (naked_dna_stretches, right_extreme - last_capital) # check if -1 is required
        }
    }else {
...
assign_binding_states <- function (overlap_string = "",
    ...
    #stop("here")
    fp_info_query = fp_info_for_sub_molecule (query_footprint = intersected_fp_str,
    ...
    left_neighbor = extend_fp_from_center(sub_moleculue = intersected_m_str,
    ...
    right_neighbor = extend_fp_from_center(sub_moleculue = intersected_m_str,
                                           complete_molecule = complete_m_str,
                                           abs_roi_start = abs_roi_start,
                                           abs_roi_end = abs_roi_end,
                                           abs_complete_molecule_start = abs_read_start,
    ...
    total_percent_fp_in_intersect = round ((str_count(intersected_fp_str, "F")/intersect_len)*100, 3)
    labels_on_reads = c()
    label = "NA"
    labelled = "NA"
    ...
    if (length(indiv_fp_percent) == 0){
        ...
        max_percentage_overlap = indiv_fp_percent[max_contribution_in_the_intersect]
        length_of_max_contribution = fp_info_query$fp_len_query[max_contribution_in_the_intersect]
        abs_start_loc_of_max = fp_info_query$fp_start_loci_on_query[max_contribution_in_the_intersect]
        abs_fp_start_on_complete_fp = fp_info_query$abs_fp_start_in_query[abs_start_loc_of_max]
        if (total_percent_fp_in_intersect <= 30  && percentage_methylation > 25){
            labelled = paste(overlap_string, intersected_fp_str, max_percentage_overlap,
                             length_of_max_contribution,
                             label_list$Naked_DNA, total_length, sep = "\t")
            label = "Naked_DNA"
        }else if (total_percent_fp_in_intersect <=30 && percentage_methylation <=25){
            labelled = paste(overlap_string, intersected_fp_str, max_percentage_overlap,
                             length_of_max_contribution,
                             label_list$Nuc, total_length, sep = "\t")
            label = "Nuc"
        }else if (total_percent_fp_in_intersect > 30 && length_of_max_contribution <=fp_cap){
            #print (c( abs_fp_start_on_complete_fp,
            #          rel_roi_start,
            #          abs_start_loc_of_max,
            #          length_of_max_contribution))
            if ( (abs_fp_start_on_complete_fp + length_of_max_contribution - 1) == total_length || abs_fp_start_on_complete_fp == 1) {
                labelled = paste(overlap_string, intersected_fp_str, max_percentage_overlap,
                                 length_of_max_contribution,
...
getHubURL <- function (organism = "dmelanogaster",
    ...
    row.names (hubs) = paste(hubs$Organism, hubs$model,
                             hubs$Condition, hubs$Type, hubs$Genome, hubs$Track, sep = "@")

    identifier = paste(organism, model, condition, type,
                       genome_assembly, tr, sep = "@")
    track_url = hubs[identifier, "Hub_URL"]
    track_url = paste0("https://api.genome.ucsc.edu/getData/track?hubUrl=",track_url)
    return (track_url)
}
remove_duplicate_molecules <- function (fp_file = "peak229.all_fp.bed", label = "peak229_final"){
    # the data is coming from big bed file, so the coordinates
    # are ought to be sorted
    dt = read.table(fp_file, sep = "\t", header = F, stringsAsFactors = F)
    dt ["srr"] =  unlist (lapply (dt$V4, function (x) { unlist (strsplit (x, split = "\\."))[[1]]}))
    dt["mol"] = unlist (lapply (dt$V4, function (x) {unlist (strsplit (x, split = "`"))[[2]]}))
    idx_duplicate = which (duplicated(dt[, c ("V1", "V2", "V3", "srr", "mol")]))
...
plotFootprints <- function (organism = "dmelanogaster", model = "S2",
                            ...
                            tr = "fp_and_mvec", label = "peak229_final",
                            span_left = 150, span_right = 150, remove_dup = F,
                            fp_cap = 50){
    # get JSON first
    #print (track)
    ...
    json_file = downloadJson(track_url = track_url, chromosome = chromosome,
                             assembly = genome_assembly, st = start, en = stop, label = label, tr = tr)
    tmp_file = JSON_to_Bed(json_file = json_file, label = label, tr = tr)
    subject_file = NA
    if (remove_dup == T){
        subject_file = remove_duplicate_molecules(fp_file = tmp_file, label = label)
    }else{
    ...
    cat ("\n", file = paste0(label, ".bed"), append = T)
    ...
    read_details = unlist (lapply(fp_and_mvec_info,
                                  function (x) {
                                      yt = unlist(strsplit(x, split = "\\|"))[1] }))
    fp_vec = unlist (lapply(fp_and_mvec_info,
                            function (x) {
                                yt = unlist(strsplit(x, split = "\\|"))[2] }))
    ...
    with_edges = unlist (apply(combined, 1,
                               function(x){add_footprints_on_edges(x[1], x[2])} ))
    ...
    for (idx in seq (nrow(processed_df)) ){
        ...
        cat (assigned_states$labelled, file = ofp, append = T)
        cat("\n", file = ofp, append = T)
        ...
        marker = paste0(chrQ,":", abs_roi_start, "-", abs_roi_end, "`",
                        read_id, "#", label_id)
        extended_fp = extend_fp_from_center(sub_moleculue = sub_molecule_fp,
        ...
        extended_m = extend_fp_from_center(sub_moleculue = sub_molecule_m,
        ...
        if (length(fp_info$just_length_vec) > 0){
            ...
            abs_start_vec = c (abs_start_vec, fp_info$fp_start_locus[which_max])
    ...
    all_valid_states$state_label = unlist(
        lapply (all_valid_states$state,function (x){state_info[as.character(x)]}))
    ...
    write.table(all_valid_states, file.path(plot_dir, paste0(label, ".states.tsv")),
    ...
    if (nrow(jj) > 15){
        ...
        for (i in rev(seq (nrow(all_valid_states)) )){
            ...
            if (counter < nrow(all_valid_states)) {
                segments(-30, (line_loc + 0.5), x_width, (line_loc + 0.5), lw = 0.5)
            }
            if (counter == 1) {
        ...
        segments((span_left - 15) , 0, (span_left - 15),
                 (total_molecules + 0.5), lw = 0.5, col = "Red")
        segments((span_left + 15), 0, (span_left + 15),
                 (total_molecules + 0.5), lw = 0.5, col = "Red")
        ticks_at = seq(0, span_left + span_right, 50)
        lab_at_ticks = as.character(ticks_at - as.integer((span_left + span_right)/2))
        axis(1, ticks_at,
             lab_at_ticks,
        ...
        saveToPDF(file.path(plot_dir, paste0(label, ".plot.pdf")), height = 6, width = 4.5)
        saveToPNG(file.path(plot_dir, paste0(label, ".plot.png")), height = 6, width = 4.5,
                  units = "in", res = 300)
        saveToEPS(file.path(plot_dir, paste0(label, ".plot.eps")), height = 6, width = 4.5,
                  horizontal = F, paper = "special")
    }else {

        par(mgp=c(1.5,0.25,0), cex = 0.75)
        plot(c(0, 1), c(0, 1), ann = F, bty = 'n', type = 'n', xaxt = 'n', yaxt = 'n')
        text(x = 0.5, y = 0.5, paste0("Only ", nrow (jj), " molecules, not enough for plotting"))
